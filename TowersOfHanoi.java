...
public class TowersOfHanoi extends JFrame implements ActionListener, Runnable {
  ...
  int fwidth = 1000, fheight = 800; // size of JFrame

  // Formatting the JLabel
  ...
  JLabel current_move = new JLabel("Present Move : ");
  ...
  public TowersOfHanoi() {
    ...
    // size/location buttons, boxes, questions
    question.setBounds(400, 560, 300, 25);
    start.setBounds(300, 520, 100, 25);
    exit.setBounds(600, 520, 100, 25);
    numof_moves.setBounds(100, 50, 300, 25);
    current_move.setBounds(300, 50, 300, 25);

    // add buttons, boxes, questions
    add(question);
    add(start);
    add(exit);
    add(numof_moves);
  ...
  public void Hanoi(int diskCount, int from, int dest, int by) {
    if (diskCount == 1) {
      ...
      try {
        ...
        current_move.setText(
            "Current Move : Disk " + (num + 1) + " moved from " + (char) (from + 64) + " --> " + (char) (dest + 64));
      } catch (Exception e) {
      }
    } else {
